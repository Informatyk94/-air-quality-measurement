...
//Global variables
int pin = 4; //Input pin from VOUT1 from dust sensor, abbreviation for pin D8
...
const char* wifi_password = "router123";
...
void setup() {
  ...
  pinMode(4, INPUT); // Setup pin as input for reading low time
...
void wifiSendData(double concentration, double ratio, double mass){
  ...
  if (!client.connect("pomiary.cholewa.lk.pl", httpPort)) {
  ...
  String url = "/input?node=" + String(node) + "&concentration=" + String(concentration) +  "&ratio=" + String(ratio) +  "&apikey=c3cad7f4bdad9950432c37fa573c9b65"; //Remember to correct API key
        
  String host = "Host:  pomiary.cholewa.lk.pl\n";
...
//  url = " /input?node=" +  String(node) + "&json={concentration:" + String(concentration) +  ",ratio:" + String(ratio) +  "}&apikey=c3cad7f4bdad9950432c37fa573c9b65"; //Remember to correct API key
//  
//  // This will send the request to the server
